...
if(isset($_GET['date'])){
    $date = $_GET['date'];
    $stmt = $mysqli->prepare("select * from booking_a102 where date = ?");
...
if(isset($_POST['submit']))
    ...
    $stmt = $mysqli->prepare("select * from booking_a102 where date = ? AND timeslot = ?");
    ...
    $stmt = $mysqli->prepare("INSERT INTO booking_a102 (name, emailid, department, event_name, date, timeslot) VALUES (?,?,?,?,?,?)");
    $stmt->bind_param('sssssss', $name, $emailid, $department, $event_name, $date, $timeslot);
    $stmt->execute();
    $msg = "<div class='alert alert-success'>Booking Form Submitted Successfully</div>";
    $booking_a102[]=$timeslot;
    $stmt->close();
    $mysqli->close();
    }
    }
  ...
  <body>
   ...
   <div class="col-md-12">
    ...
    <div class="col-md-2">
        ...
        <?php if(in_array($ts, $booking_a102)){ ?>
